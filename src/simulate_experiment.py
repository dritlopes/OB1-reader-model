...
from reading_functions import get_threshold, string_to_ngrams_and_locations, build_word_inhibition_matrix, \
    ...
    sample_from_norm_distribution, find_word_edges, get_midword_position_for_surrounding_word, calc_word_attention_right, update_threshold
...
def compute_words_activity(stimulus, lexicon_word_ngrams, eye_position, attention_position, attend_width, pm, fixation_data, word_overlap_matrix, tokens, fixation, lexicon_word_activity):
    ...
    # all stimulus bigrams used, therefore the same inhibition for each word of lexicon (excit is specific to word, inhib same for all)
    ...
    for lexicon_ix, lexicon_word in enumerate(lexicon_word_ngrams.keys()):
        ...
        ngram_intersect_list = set(unit_activations.keys()).intersection(lexicon_word_ngrams[lexicon_word])
    ...
    n_ngrams_lexicon = len(set(itertools.chain([list_ngrams for list_ngrams in lexicon_word_ngrams.values()])))
    word_input = word_input / np.array(n_ngrams_lexicon)
    ...
    lexicon_word_inhibition = np.dot((overlap_select ** 2), -(lexicon_select ** 2)) / np.array(n_ngrams_lexicon)
...
def match_active_words_to_input_slots(order_match_check, stimulus, recognized_position_flag, recognized_word_at_position_flag, recognized_word_at_position, above_thresh_lexicon, lexicon_word_activity, lexicon, min_activity, stimulus_position):
    ...
    for slot_to_check in range(n_words_in_stim):
        ...
        if not recognized_position_flag[word_index]:
            ...
            recognized_words_fit_len = above_thresh_lexicon * np.array([int(is_similar_word_length(len(x.replace('_', '')),len(word_searched))) for x in lexicon])
...
def simulate_reading(pm,tokens,word_overlap_matrix,lexicon_word_ngrams,lexicon_word_index,total_n_words,lexicon_thresholds_dict,lexicon,pred_values,tokens_to_lexicon_indices):
    ...
    # max pred to compute new threshold based on pred also
    max_predictability = max(pred_values.values())

    # Initialise variables unique to continuous reading
    # if task == 'continuous reading':
    regression = False
    wordskip = False
    refixation = False
    forward = False
    ...
    saccade_error = 0
    refixation_type = 0 # 1 refixation not recognized, 2 refixation by activity
    wordskip_pass = 0
    saccade_type_by_error = 0
    offset_previous = 0
    allocated_dict = defaultdict(list)
    # TODO figure out what they are doing
    ...
    while not end_of_task:
        ...
        print(f'---Fixation {fixation_counter+1}---')
        ...
        # stimulus is defined according to fixation in continuous reading. Otherwise, stimulus is pre-defined
        if task != 'continuous reading':
        ...
        # NV: the order list should reset when stimulus changes or with the first stimulus, therefore they are computed within the cycle loop
        ...
        right_word_edge_letter_indices, left_word_edge_letter_indices, fixation_first_position_right_to_middle, fixation_first_position_left_to_middle = \
            find_word_edges(fixation_center, eye_position, stimulus, tokens, fixation) # TODO fix this function, wrong indices
        ...
        if pm.prediction_flag:
            ...
            fix_end = (fixation + 2) if (fixation < total_n_words) else fixation
            ...
            for position_in_text in lexicon_fixated_words:
                ...
                lexicon_thresholds[position_in_lexicon] = update_threshold(position_in_text, lexicon_thresholds[position_in_lexicon], max_predictability, pm.wordpred_p, pred_values)
        ...
        while amount_of_cycles_since_attention_shifted < 5:
            ...
            stim_activity = sum([lexicon_word_activity[lexicon_word_index[word]]
                                 for word in stimulus.split() if word in lexicon_word_index.keys()])
            ...
            fixation_data['exact recognized words positions'].append(above_thresh_lexicon[0][:])
            fixation_data['exact recognized words'].append([lexicon[i] for i in above_thresh_lexicon[0]])
            ...
            if not shift:
                ...
                if recognized_position_flag[fixation]:
                    ...
                    shift_start = sample_from_norm_distribution(pm.mu, pm.sigma, pm.distribution_param,
                                                                recognized=True)
                ...
                if amount_of_cycles >= shift_start:
                    ...
                    if regression:
                        if lexicon_word_index[tokens[fixation + 1]] in \
                                fixation_data['recognized words indices']:
                            # go forward two words
                            attention_position = get_midword_position_for_surrounding_word(2,
                            ...
                            # go forward one word
                            attention_position = get_midword_position_for_surrounding_word(1,
                                                                                           right_word_edge_letter_indices,
                                                                                           left_word_edge_letter_indices)
                            forward = True
                    ...
                    elif fixation > 1 and not recognized_position_flag[fixation - 1] and not regression_flag[
                        fixation - 1]:
                        attention_position = get_midword_position_for_surrounding_word(-1,
                                                                                       right_word_edge_letter_indices,
                                                                                       left_word_edge_letter_indices)
                    ...
                    # Refixate if the foveal word is not recognized but is still being processed.
                        ...
                        word_reminder_length = right_word_edge_letter_indices[0][1] - (
                        right_word_edge_letter_indices[0][0])
                    ...
                    # Perform normal forward saccade (unless at the last position in the text)
                        ...
                        if next_fixation == 0:
                            ...
                            attention_position = get_midword_position_for_surrounding_word(next_fixation,
                                                                                           right_word_edge_letter_indices,
                                                                                           left_word_edge_letter_indices)
                            if next_fixation == 1:
                                forward = True
        ...
        fixation_data['word threshold'] = lexicon_thresholds[tokens[fixation]]
        ...
        if fixation == len(tokens) - 1:  # Check if end of text is reached.
            end_of_task = True
            print("END REACHED!")
            continue
        ...
        if not refixation:
            assert(offset_from_word_center == 0)
        ...
        if next_eye_position >= len(stimulus) - 1:
            ...
            assert(right_word_edge_letter_indices[-1][1] == len(stimulus) - 2)
