...
class TaskAttributes:
    ...
    def __init__(self, task_name, stim_name, stim, stimAll, language,
                 ...
                 are_nonwords = False,
                 display_overlap_inhib_matrix=False,
        ...
        self.are_nonwords = are_nonwords
        ...
        self.display_overlap_inhib_matrix = display_overlap_inhib_matrix
...
def return_attributes(task_to_run, language, stimuli_filepath, file_separator):
 ...
 assigns 'German' to the language attribute and returns an instance of TaskAttributes with the associated attributes."""
    ...
    stim_data, _ = get_stimulus_text_from_file(stimuli_filepath, file_separator)
    ...
    if task_to_run == 'continuous_reading':
        ...
        stim_data = pd.read_csv('stimuli\EmbeddedWords_stimuli_all_csv.csv', delimiter=';')
...
def return_task_params(task_attributes):
    ...
    # word activation
    bigram_to_word_excitation = 1.0 # was 1.65 #1  # 0.0044 # 3.09269333333 # 2.18 # inp. divded by #ngrams, so this param estimates excit per word [diff from paper] 1.65 for EmbeddedWords, 2.18 for classification and transposed
    ...
    word_inhibition = -0.5 # was -1.2 #-.4 # -0.01  # -0.016093 # -0.002
    min_activity = 0.0
    max_activity = 1.0 #1.0 # 1.3
    decay = -0.1 # was -0.05 #-0.09 # -0.06 # AL: decay in word activation over time
    discounted_Ngrams = 5 # was 10 # MM: Max extra wgt bigrams do to edges in 4-letter wrd w. gap 3. Added to bigram count in compute_input formula to compensate
    ...
    min_overlap = 0 #2 # was 2 # min overlap for words to inhibit each other. MM: unnecessary, can be deleted later

    # threshold parameters
    max_threshold = 0.5 # was 1 #0.6 # mm: changed because max activity changed from 1.3 to 1
    ...
    # attention
    attend_width = 5.0 # Was 15 # 5.0 for natural reading # 8.0  # NV: #!!!: was set to 15 for flanker, 20 for sentence and 3 for transposed
    ...
    salience_position = 1. # 1.29 (paper) # was 4.99
    ...
    # sacc_optimal_distance = 9.99 # 8 # 9.99  # 3.1 # 7.0 # 8.0 (optimal) # 7.0 (paper)
    # saccErr_scaler = 0.2  # to determine avg error for distance difference
    # saccErr_sigma = 0.17  # basic sigma
    # saccErr_sigma_scaler = 0.06  # effect of distance on sigma
